...
class NewsConnector(IDataProvider):
    """
    News connector with RNS support for UK stocks:
    1. RNS (Regulatory News Service) - UK stocks priority
    2. DuckDuckGo Search
    3. Google News RSS
    """
    ...
    def _fetch_rns_news(self, ticker: str, max_items: int) -> List[NewsItem]:
        """Fetch RNS (Regulatory News Service) announcements for UK stocks"""
        try:
            # Clean ticker (remove .L)
            clean_ticker = ticker.replace(".L", "")
            
            # RNS search via Google (RNS doesn't have public API)
            ...
            for item in items[:max_items]:
                ...
                if title_match:
                    ...
                    # Clean up RNS title
                    title = re.sub(r'\s*-\s*RNS.*$', '', title, flags=re.IGNORECASE)
            ...
            print(f"RNS fetch error: {e}")
            return []
    
    def _fetch_rns_alternative(self, ticker: str, max_items: int) -> List[NewsItem]:
        """Alternative RNS search method"""
        try:
            clean_ticker = ticker.replace(".L", "")
            
            # Search for company name + RNS
            company_names = {
            ...
            print(f"RNS alternative error: {e}")
            return []
    
    def _classify_rns_announcement(self, title: str) -> str:
        """Classify RNS announcement type"""
        title_lower = title.lower()
        
        if any(w in title_lower for w in ['result', 'earnings', 'trading update', 'financial']):
            return "FINANCIAL"
        elif any(w in title_lower for w in ['dividend', 'distribution']):
            return "DIVIDEND"
        elif any(w in title_lower for w in ['director', 'dealings', 'insider']):
            return "DIRECTOR DEALINGS"
        elif any(w in title_lower for w in ['acquisition', 'merger', 'takeover']):
            return "M&A"
        elif any(w in title_lower for w in ['buyback', 'own shares', 'purchase']):
            return "SHARE BUYBACK"
        else:
            return "REGULATORY"
    
    def _analyze_rns_sentiment(self, title: str) -> str:
        """Analyze sentiment of RNS announcement"""
        title_lower = title.lower()
        
        # Positive indicators
        positive = [
            'beat', 'growth', 'increase', 'rise', 'profit', 'gain', 'upgrade',
            'strong', 'outperform', 'record', 'higher', 'improve', 'success',
            'dividend increase', 'buyback', 'acquisition'
        ]
        
        # Negative indicators
        negative = [
            'miss', 'loss', 'decline', 'drop', 'fall', 'warning', 'downgrade',
            'weak', 'decrease', 'lower', 'worsen', 'crisis', 'investigation',
    ...
    def _get_exclude_keywords(self, ticker: str) -> List[str]:
        ...
        exclude_map = {
            "LLOY.L": ['hapag-lloyd', 'jamie lloyd', 'lloyd webber', 'shipping', 'theater'],
        }
        return exclude_map.get(ticker, [])
    
    def _get_include_keywords(self, ticker: str) -> List[str]:
        """Get include keywords for financial stocks"""
